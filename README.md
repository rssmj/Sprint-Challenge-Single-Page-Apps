...
<!-- Client-side routing runs the request to a website in the browser. When users navigate to a different portion of the site, the client doesn't send another request to the site's server. The initial response is processed and when navigating through the site, requests are handled with resources already present in the browser. -->
